...
def get_emission_prob(word,state, emission_dict, vocabLen, laplace = True):
  emission_prob = 0.0
  if laplace:
    ...
    emission = emission_dict[state][word] / sum(emission_dict[state].values())
  return emission
...
#helper function for get word for tuple
def getword_tuple(data):
  list_word = []
  for i in range(len(data)):
    list_word.append(data[i][0])
  return list_word
...
class NamedEntityRecognitionHMM:
  ...
  def generate_probabilities(self, data, laplace=True): 
    """
    ...
    #fill the first column of table of start state
    for i in range(NumState):
      state = labels[i]
      emission_prob = get_emission_prob(data[0], state, self.emissions, NumVocab, True)
    ...
    for j in range(1, Numtoken):
      ...
      dict_state_state = {}
      for i in range(NumState):
        emission_prob = get_emission_prob(word, labels[i], self.emissions, NumVocab, True)
        for k in range(NumState):
          ...
          # find the max of each state given word
          if prob > prob_word_state[i, j]:
            prob_word_state[i, j] = prob
            prob_state_state[i,j] = k
            
            
        # add the best state to state into dictionary
        dict_state_state[labels[i]] = labels[prob_state_state[i,j]]

        # add the best state to word into dictionary
        dict_prob_emssion[labels[i]] = prob_word_state[i, j]

      #add dictionaries of states to probabilities per word
      list_state_prob.append(dict_prob_emssion)

      #add dictionaries of states to to
      list_state_state.append(dict_state_state)
...
class NamedEntityRecognitionMEMM:
  ...
  def train(self, examples, iterate = 2, learning_rate=0.1):
    """
    Trains this model based on the given input data
    params: examples - a list of lists of (token, label) tuples
    return: None
    ...
    #     listword = getword_tuple(sentence)
    #     feature = self.featurize(listword)

    #     featurized = np.array(feature + [1]) # add the bias

    #     #calculate y_hat
    #     for i in range(len(self.state)):
    #       self.z.append(np.dot(featurized, self.weights[i].T))
        
    #     #calculate  gradients
    #     for s in range(len(self.state)):
    #       self.y_hat.append(softmax(self.z[s], self.z))
...
if __name__ == "__main__":
  ...
  Gold_labels = [('Colorectal', 'O'), ('polyps', 'O'), ('should', 'O'), ('be', 'O'), ('removed', 'O'), ('endoscopically', 'O'), ('whenever', 'O'), ('possible', 'O'), ('.', 'O')]
  Labeled_as =[('Colorectal', 'I'), ('polyps', 'O'), ('should', 'O'), ('be', 'O'), ('removed', 'O'), ('endoscopically', 'O'), ('whenever', 'O'), ('possible', 'O'), ('.', 'O')]
  precision(Gold_labels, Labeled_as)

  NER_MEMM = NamedEntityRecognitionMEMM()
  NER_MEMM.train(training_examples, 10, 0.1)
  # print(NER_MEMM.featurize(data))
